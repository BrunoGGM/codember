...
//Probablemente pueda hacerce mejor, reducir un bucle o algo, pero funciona :X
const analyzeKeys = (array) => {
  let regex = /^(\d+)-(\d+) (\w): (.+)$/;
  let invalidKeys = [];
  array.forEach(e => {
    ...
    if (!(totalLetters >= min && totalLetters <= max)) {
      invalidKeys.push(key);
    }
  });
  return invalidKeys[41];
}

let keys = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
